...
from utils import SQL2TextPipeline
from ChatUI import setup_ChatUI_sidebar
from pages.Model_Settings import obtain_configs
from prompts import SQL_MSG, AI_MSG , SUFFIX_ERROR_V1, SUFFIX_ERROR_V2
...
class Application():
    ...
    def obtain_user_feedback(self)-> None:
        ...
        def check_query_validity(sql_query: str)-> bool:
            """check query is executable and doesnt contain write operations"""
        ...
        try:
            ...
            self.verify_llm_status(isModelAvailable = isModelAvailable , llm = 'isResponseModelOnline', isHumanFeedback = False)
    ...
    def generate_sql_response(self, mode: str, input: str, task: str, db_schema: Union[str,None] = None)-> bool:
        ...
        try:
            ...
            if task == "regen":
                for _ in range(2):
                    sql_history.pop()
            # logger.debug(f"Prompt_SQL: {self.sql_dict['prompt']}") #(standard_mode)
            # logger.debug(f"Prompt_SQL: {sql_prompt_with_keys.format(**keys)}") #(schema_mode)(need to pass keys into arg)
            ...
            logger.info(f"Question : {input}")
            logger.info(f"SQL Query : {sql_results}")
            logger.info(f"Inference time used for sql generation: {end_1-start_1} seconds\n")
            return True
        except Exception as error:
            messages_state.pop()
    ...
    def generate_human_response(self, sql_query: str)-> bool:
        ...
        try:
            ...
            logger.info(f"SQL Results : {sql_results}")
            logger.info(f"Human-Readable Response : {self.human_response}")
            logger.info(f"Inference time used for response generation: {end_2-start_2} seconds\n")
            return True
        except Exception as error:
...
def main()-> None:
    ...
    try:
        ...
        logger.error(f"Setup Fail. Error: {error}") 
        st.warning(f"**Error**: {error}",icon="⚠️")
        return
