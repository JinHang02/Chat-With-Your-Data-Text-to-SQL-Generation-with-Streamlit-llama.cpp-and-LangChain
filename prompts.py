#texts for frontend UI
SQL_MSG = "Hello! I'm a SQL assistant that can generate SQL queries from natural language descriptions. Please provide me with your query in plain English, and I'll translate it into SQL for you."
AI_MSG = "Hello! I'm an AI assistant specializing in summarizing SQL queries and their results. Let me help bridge the gap between complex data and clear insights!"
...
STREAM_MSG_SCHEMA_MODE= "**Here is the generated SQL Query.**  \n Reply with instructions:  \n - \"regen\" if wants to regenerate sql query.  \n - new question if you are satisfied with the query!  \n ```  \n "
MODEL_SETTINGS_MD = ":exclamation: :grey[**You may modify the prompt, but please keep all placeholders (eg: :red[{input}] ) intact to ensure proper output generation**]"
SCHEMA_MODE_MD = ":exclamation: :grey[**Database schema mode only generates SQL queries from natural language and does not require any connection to database. You are required to provide the database schema to use this mode.**]"
...
SCHEMA_PROMPT = """CREATE TABLE products (
